...
public class WalletStorage {
    ...
    public void importingWalletsDetector(MainActivity c) {
        ...
        File[] wallets = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/Ether-1/").listFiles();
        if (wallets == null) {
    ...
    private boolean exportWallet(Activity c, boolean already) {
        ...
        if (ExternalStorageHandler.hasPermission(c)) {
            File folder = new File(Environment.getExternalStorageDirectory(), "Ether-1");
