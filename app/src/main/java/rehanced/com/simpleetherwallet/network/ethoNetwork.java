...
public class ethoNetwork implements NetworkAPI {
    ...
    public void getPriceChart(long starttime, int period, boolean usd, Callback b) throws IOException {
        String url = null;
        switch (period) {
            case 300: url = apiUrl + "returnChartData?fsym=ETHO&tsym=" + (usd ? "USD" : "BTC") + "&period=histominute&limit=1440";
            break;
            case 1800: url = apiUrl + "returnChartData?fsym=ETHO&tsym=" + (usd ? "USD" : "BTC") + "&period=histohour&limit=168";
            break;
            case 14400: url = apiUrl + "returnChartData?fsym=ETHO&tsym=" + (usd ? "USD" : "BTC") + "&period=histoday&limit=30";
            break;
            default:
                url = apiUrl + "returnChartData?fsym=ETHO&tsym=" + (usd ? "USD" : "BTC") + "&period=histoday&limit=365";
        }
